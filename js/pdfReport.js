...
export function generatePerformanceReport(results, certInfo) {
    ...
    const certLabel = certInfo?.name
        || document.getElementById('sidebar-cert-label')?.innerText
        || results.certId?.toUpperCase() || 'AWS';
    ...
    function newPageIfNeeded(needed = 10) { if (y + needed > pageH - 15) { doc.addPage(); y = 15; } }
    function fillRect(x, ry, w, h, r, g, b) { doc.setFillColor(r, g, b); doc.rect(x, ry, w, h, 'F'); }
    ...
    doc.text(`Certificacao: ${certLabel}   |   Pontuacao: ${awsScore} / 1000   |   Data: ${dataHoje}`, marginL, 22);
    y = 38;

    // 2. DOMÍNIOS
    black(); doc.setFont('helvetica', 'bold'); doc.setFontSize(12);
    doc.text('Desempenho por Dominio', marginL, y);
    y += 2; doc.setDrawColor(249, 115, 22); doc.setLineWidth(0.5);
    doc.line(marginL, y, pageW - marginR, y); y += 6;

    const hasDomainsData = results.domainScores && Object.keys(results.domainScores).length > 0;
    ...
    if (hasDomainsData && hasDomainDefs) {
        certInfo.domains.forEach(domain => {
            ...
            const isOk = pct >= 70;
            const bgR = isOk ? 240 : 254, bgG = isOk ? 253 : 242, bgB = isOk ? 244 : 242;
            const lcR = isOk ? 22 : 220, lcG = isOk ? 163 : 38, lcB = isOk ? 74 : 38;

            fillRect(marginL, y, contentW, 20, bgR, bgG, bgB);
            doc.setFillColor(lcR, lcG, lcB); doc.rect(marginL, y, 2.5, 20, 'F');
            black(); doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
            doc.text(wrap(domain.name, contentW - 35, 10)[0], marginL + 5, y + 7);
            rgb(lcR, lcG, lcB); doc.setFontSize(12);
            doc.text(`${pct.toFixed(0)}%`, pageW - marginR - 2, y + 8, { align: 'right' });

            const bx = marginL + 5, by = y + 12, bw = contentW - 30;
            doc.setFillColor(220, 220, 220); doc.rect(bx, by, bw, 3, 'F');
            doc.setFillColor(lcR, lcG, lcB); doc.rect(bx, by, bw * Math.min(pct / 100, 1), 3, 'F');

            rgb(100, 100, 100); doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
            doc.text(`${sd.correct} de ${sd.total} corretas   ${isOk ? '| APROVADO' : '| ATENCAO'}`, marginL + 5, y + 18);
            y += 24;
        });
        ...
        Object.entries(results.domainScores).forEach(([domainId, sd]) => {
            ...
            doc.setFillColor(lcR, lcG, lcB); doc.rect(marginL, y, 2.5, 14, 'F');
            black(); doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
            doc.text(String(domainId), marginL + 5, y + 6);
            rgb(lcR, lcG, lcB); doc.setFontSize(10);
            doc.text(`${pct.toFixed(0)}%  (${sd.correct}/${sd.total})  ${isOk ? 'APROVADO' : 'ATENCAO'}`, pageW - marginR - 2, y + 6, { align: 'right' });
            y += 17;
        });
    } else {
        rgb(150, 150, 150); doc.setFontSize(9); doc.setFont('helvetica', 'italic');
        doc.text('Dados de dominio nao disponiveis para este relatorio.', marginL, y); y += 8;
    }
    ...
    if (!results.answers || results.answers.length === 0) {
        rgb(150, 150, 150); doc.setFontSize(9);
        ...
        results.answers.forEach((ans, index) => {
            ...
            const blockH = qLines.length * LH + 8 + uLines.length * LH + 14 +
                (isCorrect ? 0 : cLines.length * LH + 14) + eLines.length * LH + 14;
            ...
            // Resposta do usuário
            const uR = isCorrect ? 22 : 220, uG = isCorrect ? 163 : 38, uB = isCorrect ? 74 : 38;
            fillRect(marginL, y, contentW, uLines.length * LH + 11, isCorrect ? 240 : 254, isCorrect ? 253 : 242, isCorrect ? 244 : 242);
            doc.setFillColor(uR, uG, uB); doc.rect(marginL, y, 2.5, uLines.length * LH + 11, 'F');
            rgb(100, 100, 100); doc.setFont('helvetica', 'bold'); doc.setFontSize(7);
            doc.text(`SUA RESPOSTA [${isCorrect ? 'CORRETA' : 'INCORRETA'}]`, marginL + 5, y + 5);
            rgb(uR, uG, uB); doc.setFont('helvetica', 'bold'); doc.setFontSize(9);
            ...
            if (!isCorrect) {
                ...
                doc.setFillColor(22, 163, 74); doc.rect(marginL, y, 2.5, cLines.length * LH + 11, 'F');
                rgb(22, 101, 52); doc.setFont('helvetica', 'bold'); doc.setFontSize(7);
                doc.text('RESPOSTA OFICIAL', marginL + 5, y + 5);
                rgb(21, 128, 61); doc.setFont('helvetica', 'bold'); doc.setFontSize(9);
            ...
            doc.setFillColor(59, 130, 246); doc.rect(marginL, y, 2.5, eLines.length * LH + 11, 'F');
            rgb(30, 64, 175); doc.setFont('helvetica', 'bold'); doc.setFontSize(7);
            doc.text('EXPLICACAO', marginL + 5, y + 5);
            rgb(30, 58, 138); doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
    ...
    for (let p = 1; p <= total; p++) {
        doc.setPage(p);
        rgb(170, 170, 170); doc.setFont('helvetica', 'normal'); doc.setFontSize(7);
        doc.text(`Simulador AWS  |  ${dataHoje}  |  Pagina ${p} de ${total}`, pageW / 2, pageH - 7, { align: 'center' });
    }
